# regen-final-v2.ps1 - escritura línea por línea para control total de whitespace
...
foreach ($p in $products) {
    ...
    foreach ($c in $p.Colores) {
        $yamlLines += "  - nombre: `"$($c.nombre)`""
        $yamlLines += "    hex: `"$($c.hex)`""
        $yamlLines += "    imagen: `"$($c.imagen)`""
    }
    ...
    # Construir líneas del frontmatter
    $lines = @()
    $lines += "---"
    $lines += 'nombre: "' + $nom + '"'
    $lines += "categoria: $($p.Categoria)"
    if ($p.Subtipo) { $lines += "subtipo: $($p.Subtipo)" }
    $lines += 'coleccion: "Coleccion 2026"'
    $lines += "precio: $($p.Precio)"
    $lines += "destacado: false"
    $lines += "colores:"
    $lines += $yaml
    $lines += '  tallas: ["' + ($p.Tallas -join '", "') + '"]'
    $lines += "  tallaUnica: $tallaUnicaStr"
    $lines += "  tallasConsultar: $tallasConsultarStr"
    $lines += 'tallas: ["XS", "S", "M", "L", "XL"]'
    $lines += 'descripcion_es: |'
    $lines += '  ' + $p.Nombre + ' - pieza disenada en Colombia por Abril Pijamas y Lenceria. Tejidos suaves, acabados con cuidado.'
    $lines += 'descripcion_en: |'
    $lines += '  ' + $p.Nombre + ' - piece designed in Colombia by Abril Pijamas and Lingerie. Soft fabrics, careful finishes.'
    $lines += "---"
    $lines += ""
    $lines += "# " + $p.Nombre
    $lines += ""
    $lines += "Pieza de la Coleccion 2026 de Abril Pijamas y Lenceria. Hecha a mano en Colombia."
    
    $content = ($lines -join "`n")
    Set-Content "C:\Users\SAMUEL\Projects\abril-catalogo\src\content\products\$($p.Slug).md" $content -Encoding utf8 -NoNewline
    $created++
}
