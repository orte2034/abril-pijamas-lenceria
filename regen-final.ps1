...
# 1. Build folder -> files map
$folderFiles = @{}
Get-ChildItem $root -Recurse -File | ForEach-Object {
  $folder = $_.Directory.FullName.Replace((Get-Item $root).FullName + '\','').Replace('\','/').ToLower()
  if (-not $script:folderFiles.ContainsKey($folder)) { $script:folderFiles[$folder] = @() }
  $script:folderFiles[$folder] += $_.FullName.Replace((Get-Item $root).FullName + '\','').Replace('\','/').ToLower()
}
Write-Output "Folders indexed: $($script:folderFiles.Count)"

# 2. Load 84 MDs and match
$products = @()
Get-ChildItem "C:\Users\SAMUEL\Projects\abril-catalogo\src\content\products" -Filter *.md | ForEach-Object {
  ...
  if ($nombre -and $cat -and $precio) {
    ...
    foreach ($folder in $script:folderFiles.Keys) {
      $score = 0
    ...
    if ($bestFolder -and $bestScore -gt 0) {
      $files = $script:folderFiles[$bestFolder]
      $colores = @()
      ...
      $script:finalProducts += @{
        Nombre = [regex]::Match($c, 'nombre:\s*"([^"]+)"').Groups[1].Value
        Slug = $slug
        Categoria = [regex]::Match($c, 'categoria:\s*(\w+)').Groups[1].Value
        Subtipo = [regex]::Match($c, 'subtipo:\s*(\w+)').Groups[1].Value
        Precio = [int][regex]::Match($c, 'precio:\s*(\d+)').Groups[1].Value
        Colores = $colores
      }
    }
  }
...
Write-Output "Matched products: $($finalProducts.Count)"
...
Write-Output "MDs creados: $($finalProducts.Count)"
