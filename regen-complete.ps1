# regen-complete.ps1 - Regeneración completa: estructura + tallas en un solo paso
...
# ===== 1. BUILD TALLAS CACHE CON PROPAGACIÓN =====
...
# ===== 2. MAPEAR WEB LEAF DIRS A TALLA =====
$webLeafDirs = Get-ChildItem -Path $root -Recurse -Directory | Where-Object {
...
# ===== 4. IDENTIFICAR CARPETAS-HOJA (productos) =====
...
Write-Output "Carpetas-hoja (productos): $($leafDirs.Count)"

# ===== 5. CONSTRUIR PRODUCTOS =====
$products = @()
foreach ($d in $leafDirs) {
    $files = Get-ChildItem -LiteralPath $d.FullName -File | Where-Object { $_.Extension -in '.jpg','.jpeg','.png' }
    if ($files.Count -eq 0) { continue }
    $rel = $d.FullName.Substring($root.Length + 1).Replace('\','/').ToLower()
    $parts = $rel -split '/'
    ...
    # Slug: path completo slugificado
    $slug = Slugify $rel
    $slugAux = $slug; $counter = 2
    while ($products | Where-Object { $_.Slug -eq $slugAux }) { $slugAux = "$slug-$counter"; $counter++ }
    $slug = $slugAux
    ...
    foreach ($f in $files) {
        $i++
        $colores += @{ nombre = "Color $i"; hex = "#cccccc"; imagen = "/web/$rel/" + $f.Name.ToLower() }
    }
...
foreach ($p in $products) {
    ...
    # Construir frontmatter YAML válido
    $fm = @"
---
nombre: "$nom"
...
"@
    Set-Content "$mdDir\$($p.Slug).md" $fm -Encoding utf8 -NoNewline
    $created++
}
