# regen-final-v4.ps1 - escritura explícita carácter por carácter
...
# Tallas cache
$allDirsTallas = Get-ChildItem -Path "C:\Users\SAMUEL\Projects\abril-catalogo\public\web TALLAS" -Recurse -Directory | Sort-Object { $_.FullName.Length }
$tallasCache = @{}
foreach ($d in $allDirsTallas) {
    $rel = $d.FullName.Substring("C:\Users\SAMUEL\Projects\abril-catalogo\public\web TALLAS".Length + 1).Replace('\','/')
    $leaf = Split-Path $rel -Leaf
    $tallaRaw = ''
    if ($leaf -imatch '(?i)\s+Talla\s+(.+?)(?:\s*\\|$)') { $tallaRaw = $matches[1].Trim() }
    elseif ($leaf -imatch '(?i)^Talla\s+(.+)$') { $tallaRaw = $matches[1].Trim() }
    ...
    if ($tallaRaw) {
        $tallasCache[$cleanRel] = $tallaRaw
        foreach ($sub in $allDirsTallas) {
            $subRel = $sub.FullName.Substring("C:\Users\SAMUEL\Projects\abril-catalogo\public\web TALLAS".Length + 1).Replace('\','/')
            $subClean = $subRel -ireplace '(?i)\s+Talla\s+[^/\\]+',''
            $subClean = $subClean -ireplace '(?i)^Talla\s+[^/\\]+\s*',''
            if ($subClean -like "$cleanRel/*") { $tallasCache[$subClean] = $tallaRaw }
        }
...
foreach ($d in $leafDirs) {
    ...
    $rel = $d.FullName.Substring($webRoot.Length + 1).Replace('\','/').ToLower()
    $parts = $rel -split '/'
...
# ===== ESCRIBIR MDs USANDO STREAMWRITER PARA CONTROL TOTAL =====
$mdDir = "C:\Users\SAMUEL\Projects\abril-catalogo\src\content\products"
...
foreach ($p in $products) {
    ...
    $subLine = if ($p.Subtipo) { "subtipo: $($p.Subtipo)" } else { "" }
    
    $sw = New-Object System.IO.StreamWriter("C:\Users\SAMUEL\Projects\abril-catalogo\src\content\products\$($p.Slug).md", $false, [System.Text.Encoding]::UTF8)
    try {
        $sw.WriteLine("---")
        $sw.WriteLine('nombre: "' + $nom + '"')
        $sw.WriteLine("categoria: $($p.Categoria)")
        if ($p.Subtipo) { $sw.WriteLine("subtipo: $($p.Subtipo)") }
        $sw.WriteLine('coleccion: "Coleccion 2026"')
        $sw.WriteLine("precio: $($p.Precio)")
        $sw.WriteLine("destacado: false")
        $sw.WriteLine("colores:")
        foreach ($c in $p.Colores) {
            $sw.WriteLine("  - nombre: `"$($c.nombre)`"")
            $sw.WriteLine("    hex: `"$($c.hex)`"")
            $sw.WriteLine("    imagen: `"$($c.imagen)`"")
        }
        $tallaUnicaStr = if ($p.TallaUnica) { 'true' } else { 'false' }
        $tallasConsultarStr = if ($p.TallasConsultar) { 'true' } else { 'false' }
        $sw.WriteLine('tallasDisponibles: ["' + ($p.Tallas -join '", "') + '"]')
        $sw.WriteLine("tallaUnica: $tallaUnicaStr")
        $sw.WriteLine("tallasConsultar: $tallasConsultarStr")
        $sw.WriteLine('tallas: ["XS", "S", "M", "L", "XL"]')
        $sw.WriteLine('descripcion_es: |')
        $sw.WriteLine('  ' + $p.Nombre + ' - pieza disenada en Colombia por Abril Pijamas y Lenceria. Tejidos suaves, acabados con cuidado.')
        $sw.WriteLine('descripcion_en: |')
        $sw.WriteLine('  ' + $p.Nombre + ' - piece designed in Colombia by Abril Pijamas and Lingerie. Soft fabrics, careful finishes.')
        $sw.WriteLine("---")
        $sw.WriteLine("")
        $sw.WriteLine("# " + $p.Nombre)
        $sw.WriteLine("")
        $sw.WriteLine("Pieza de la Coleccion 2026 de Abril Pijamas y Lenceria. Hecha a mano en Colombia.")
    } finally {
        $sw.Close()
    }
    $created++
