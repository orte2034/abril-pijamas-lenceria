...
# 2. Load products - USING foreach (not pipeline)
$products = @()
$mdFiles = Get-ChildItem "C:\Users\SAMUEL\Projects\abril-catalogo\src\content\products" -Filter *.md
foreach ($f in $mdFiles) {
  ...
  if ($nombre -and $cat -and $precio) {
    $products += @{ Nombre=$nombre; Slug=$slug; Categoria=$cat; Subtipo=$sub; Precio=[int]$precio }
  }
}
Write-Output "Products loaded: $($products.Count)"
...
Write-Output "Final products: $($finalProducts.Count)"
